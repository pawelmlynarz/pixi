...
PXFRONTEND_API void DrawDashedLine(
    ...
    float DashLength = 6.0f, float GapLength = 4.0f, float Thickness = 1.0f
);
...
PXFRONTEND_API void DrawDashedWindowBorder(
    ...
    float DashLength = 10.0f, float GapLength = 2.0f, float Thickness = 1.5f
);

PXFRONTEND_API void DrawDashedHeader(
    char const* Text, ImVec2 const& Padding = {6.f, 6.f},
    ImColor LineColor = IM_COL32(160, 160, 160, 180), ImColor TextColor = IM_COL32(220, 220, 220, 255),
    float DashLength = 10.0f, float GapLength = 2.0f, float Thickness = 1.5f, float TextPaddingLeft = 0.0f
);
