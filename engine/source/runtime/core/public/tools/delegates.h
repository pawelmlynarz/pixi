...
public:
    ...
    virtual void Execute(TArgs&&... Args) override
    {
        if (StaticFuncPtr_)
        {
            m_StaticFuncPtr(Forward<TArgs>(Args)...);
        }
    }
};
...
public:
    ...
    { 
        ...
        {
            (Instance_->*ClassMemFuncPtr_)(Forward<TArgs>(Args)...);
        }
    }
};
...
public:
    explicit CallableLambdaWrapper(TLambda&& FunctorObject) noexcept
        : LambdaObject_(Forward<TLambda>(FunctorObject))
    {
    ...
    { 
        return LambdaObject_(Forward<TArgs>(Args)...);
    }
};
...
public:
    ...
    Delegate(Delegate&& Rhs) noexcept
        :   CallableWrapper_(MoveTemp(Rhs.CallableWrapper_))
    {}
    ...
    {
        CallableWrapper_ = MakeUnique<CallableLambdaWrapper<std::decay_t<TLambda>, TRetVal(TArgs...)>>(Forward<TLambda>(LambdaObject));
    } 
    ...
    void ExecuteIfBound(TArgs&&... Args)
    {
        if (CallableWrapper_)
        {
            CallableWrapper_->Execute(Forward<TArgs>(Args)...);
        }
    }
};
...
public:
    void AddStatic(TRetVal(*StaticFuncPtr)(TArgs...))
    {
        ...
        Delegates_.emplace_back(MoveTemp(Delegate_));
    }
    ...
    void Broadcast(TArgs&&... Args)
    {
        ...
        {
            Delegate_.ExecuteIfBound(Forward<TArgs>(Args)...);
        }
    }
};
